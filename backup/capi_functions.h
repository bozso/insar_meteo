...
template<class T>
static int ar_import_check(np_wrap<T>& array, const py_ptr to_convert,
        ...
        if (array_ndim != ndim) {
            PyErr_Format(PyExc_ValueError, "Array %s is %d-dimensional, but "
                                           "expected to be %d-dimensional",
                                            name, array_ndim, ndim);
            return 1;
        }
    }
    
    ar_struct.ndim = array_ndim;\
    (ar_struct).shape = PyArray_DIMS(tmp);\
    (ar_struct).strides = (npy_intp *) malloc((ar_struct).ndim * sizeof(npy_intp));\
    memcpy((ar_struct).strides, PyArray_STRIDES(tmp), (ar_struct).ndim * sizeof(npy_intp));\
    \
    for(uint ii = 0; ii < (ar_struct).ndim; ++ii)\
        (ar_struct).strides[ii] /= sizeof(*((ar_struct).data));\
    \
    (ar_struct).data = PyArray_BYTES(tmp);\
    (ar_struct).np_array = tmp;\
    (ar_struct).name = name;\
...
static int _ar_import_check(void * array, const py_ptr to_convert,
...
#define ar_import_check(ar_struct, to_convert, typenum, edim, name)\
do {\
    np_ptr tmp;\
    if(_ar_import_check(&tmp, (to_convert), (typenum),\
                        NPY_ARRAY_IN_ARRAY, (edim), (name)))\
        goto fail;\
    \
    (ar_struct).ndim = (uint) PyArray_NDIM(tmp);\
    (ar_struct).shape = PyArray_DIMS(tmp);\
    (ar_struct).strides = (npy_intp *) malloc((ar_struct).ndim * sizeof(npy_intp));\
    (ar_struct).strides = memcpy((ar_struct).strides, PyArray_STRIDES(tmp), (ar_struct).ndim * sizeof(npy_intp));\
    \
    for(uint ii = 0; ii < (ar_struct).ndim; ++ii)\
        (ar_struct).strides[ii] /= sizeof(*((ar_struct).data));\
    \
