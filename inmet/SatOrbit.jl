...
function fit_orbit(path::AbstractString, preproc::AbstractString,
    ...
    design = Matrix{Float64}(undef, ndata, deg + 1)
    ...
    fit = transpose(design \ transpose(coords))
...
function load_fit(fit_file::AbstractString, display=false::Bool)
    ...
    if display
        if centered
            ...
            println("coords_mean: ", string(coords_mean))
        end
        println("deg: ", deg)
        println("fit: ", string(fit))
        return
    end
end
