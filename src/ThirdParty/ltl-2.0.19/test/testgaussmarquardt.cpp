...
*
* ---------------------------------------------------------------------
* $Id: testgaussmarquardt.cpp 538 2014-05-28 15:20:26Z cag $
* ---------------------------------------------------------------------
*
...
int main(int argc, char **argv)
{
   try
   {
      ...
      {
         FVector<double, 10> fitpar;
         fitpar =
            10000.0, 10000.0, 1.0, 2.0, 6.0, 6.0, (25.0 * M_PI / 180.0), 1.2, 0.1, 0.2;
         Moffat<double, float, 10, 2>::fill(fitpar, A);
         A_error = sqrt(A);
         cout << "   Input parameters: "
              << fitpar << endl;
         ...
         cerr << "   Marquardt-Levenberg fitting 10-parameter Moffat ..." << endl
              << "   C + mx * x + my * y + A * pow( f(x_width, y_width," << endl
              << "                                    angle, x_0, y_0), -beta)" << endl;
         FVector<bool, 10> ignore = false;
         //ignore (7) = true;
         FVector<double, 10> startpar;
         startpar = 9990, 11000.0, 1.2, 1.8, 5.9, 6.1, (20.0 * M_PI / 180.0), 0.5, 1.0e-6, 1.0e-5;
         testmarquardt< Moffat<double, float, 10, 2>,
            double, float, 10, 2 >(startpar, ignore, A, A_error);
      }
      {
         cerr << "   Marquardt-Levenberg fitting 8-parameter Moffat ..." << endl
              << "   C + A * pow( f(x_width, y_width," << endl
              << "                  angle, x_0, y_0), -beta)" << endl;
         FVector<bool, 8> ignore = false;
         //ignore (7) = true;
         FVector<double, 8> startpar;
         startpar = 9990, 11000.0, 1.2, 1.8, 5.9, 6.1, (20.0 * M_PI / 180.0), 0.5;
         testmarquardt< Moffat<double, float, 8, 2>,
            double, float, 8, 2 >(startpar, ignore, A, A_error);
      }
   }
