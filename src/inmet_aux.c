...
{
    argp const ap = {argc, 3, argv, "test",
    "[testfile1] [arg1] [arg2]\
     \n\n"};
    ...
    if (array_new(&arr, dt_double, 1, rowmajor, (size_t[]){5}))
        return 1;
...
int calc_azi_inc(int argc, char **argv)
{
    ...
    if (check_narg(&ap) or get_arg(&ap, 4, "%u", &max_iter))
        return 1;
    
    File coords_file = open(argv[3], "rb"),
         out_file    = open(argv[6], "wb");
